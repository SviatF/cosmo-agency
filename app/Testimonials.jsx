...
const SOURCE_REVIEWS = [
  {
    lang: 'ru',
    text: 'Для меня самое сложное было начать, но я быстро втянулась в процесс благодаря твоей поддержке, спасибо большое. Надеюсь, что наше сотрудничество будет очень долгим, потому что сама работа и деньги, которые я тут получаю — просто топ ❤️',
  },
  {
    lang: 'ru',
    text: 'Я пробовала раньше себя в этой сфере, но всё было таким непонятным 🙈 А тут… спасибо за такое подробное обучение, всё просто и понятно. И за аванс — я не ожидала, что столько можно заработать за 3 дня 🥰',
  },
  {
    lang: 'ru',
    text: 'Так, ну скажу честно: на старте очень боялась и переживала, что не получится, но быстро влилась благодаря твоей поддержке и помощи 💗 На деле работа в удовольствие: удобный график, ооочень хорошая зарплата и иногда в чатах бывает супер весёлая атмосфера. Мне нравится, что можно быть самой собой и при этом зарабатывать, просто общаясь. Кстати, недавно в отпуске мне сказали, что у меня хороший английский, хотя пришла сюда, не зная его совсем — за это отдельный плюсик 🫶',
  },
  {
    lang: 'ua',
    text: 'Ти бачила взагалі ту зп? Я на машину почала відкладати нарешті 😍 Раніше могла відкласти тільки нервовий зрив до обідньої перерви 🤣 Якщо комусь потрібні відгуки, то ось мій чесний відгук: іноді буває важко, тому що за день спілкуєшся з різною кількістю людей, але це такий кайф, коли знаходиш класного співрозмовника і ще при цьому отримуєш за це гроші. Ще дякую, що підтримуєш в дні, коли я не в ресурсі ❤️‍🔥 і за своєчасну зарплату. Сподіваюсь, далі буде тільки ліпше!',
  },
  {
    lang: 'ua',
    text: 'Просто хочу сказати ДЯКУЮЮЮЮЮ ❤️ Я ніде ще не стикалася з таким відношенням від адміна, ти пречудова 😍 Про роботу можу сказати тільки одне — це суцільний кайф: спілкуєшся, заробляєш, зп без затримок, що великий плюсик вам в карму 😂 Вже відклала гроші на новий ноут і камеру.',
  },
  {
    lang: 'ua',
    text: 'Мені все подобається, від самої роботи до графіку і зп 🤗 Спочатку поєднувала з іншою роботою, а потім віддала пріоритет цій, змогла нарешті записатися в зал 😅, знайшла подружок, почала пізнавати цей світ, а не тільки дім-робота-дім. Це дуже цікавий і насичений досвід, дуже дякую за те, що тоді вмовила спробувати — ти дійсно хрещена-фея 🥰',
  },
];
...
function localizedText(review, locale) {
  if (locale === review.lang || locale === 'ru' && review.lang === 'ru' || locale === 'ua' && review.lang === 'ua') return review.text;
  if (locale === 'en') {
    const en = [
    ...
    return en[SOURCE_REVIEWS.indexOf(review)];
  }
  if (locale === 'ua') {
    const ua = [
    ...
    return ua[SOURCE_REVIEWS.indexOf(review)] || review.text;
  }
  const ru = [
    null, null, null,
  ...
  return ru[SOURCE_REVIEWS.indexOf(review)] || review.text;
}
...
export default function Testimonials({ locale = 'ru' }) {
  ...
  useEffect(() => {
    const id = window.setInterval(() => setActive((value) => (value + 1) % reviews.length), 8500);
    ...
    <section className="testimonials" id="reviews">
      <div className="testimonials__inner shell">
        <div className="testimonials__heading">
          <div><p className="eyebrow">{l.eyebrow}</p><h2>{l.title}</h2></div>
          <div className="testimonials__counter"><span>{String(active + 1).padStart(2, '0')}</span><i>/</i><b>{String(reviews.length).padStart(2, '0')}</b></div>
        </div>
        <article className="testimonial-card" key={`${locale}-${active}`}>
          <div className="testimonial-card__top"><span>{l.meta}</span><div aria-label="5 stars">✦ ✦ ✦ ✦ ✦</div></div>
          <div className="testimonial-card__quote-mark">“</div>
          <blockquote>{review.display}</blockquote>
          <div className="testimonial-card__footer">
            <div className="testimonial-card__person"><div className="testimonial-card__avatar">C</div><div><strong>{l.person}</strong><small>VERIFIED TEAM STORY</small></div></div>
        ...
        <div className="testimonials__dots" aria-label="Testimonials navigation">{reviews.map((_, index) => <button key={index} className={index === active ? 'active' : ''} aria-label={`Review ${index + 1}`} onClick={() => setActive(index)} />)}</div>
